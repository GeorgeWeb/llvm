...
int main() {
  ...
  auto bundle =
      sycl::get_kernel_bundle<kernel_vadd_and_sum,
                              sycl::bundle_state::executable>(q.get_context());
  auto kernel = bundle.template get_kernel<kernel_vadd_and_sum>();
  auto maxWGSize{kernel.template get_info<
      sycl::info::kernel_device_specific::work_group_size>(q.get_device())};
  std::cout << "TestKernel max WG size on this device: " << maxWGSize << '\n';
  auto numRegs{kernel.template get_info<
      sycl::info::kernel_device_specific::ext_codeplay_num_regs>(q.get_device())};
